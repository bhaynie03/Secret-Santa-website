...
class Account{
	...
	function getWishListItems($account_id, $viewer_id){
		...
		while($wish = $statement->fetchObject()){
			...
			$wishListItems .= "<input class='wishes' type='checkbox' name='allwishes[]' value='$wish->wish_id' /><span class='wishes2'>$wish->value $dibNames </span><br>";
		}
	...
	function getAllViewableProfiles($account_id, $event_id, $buddy_id){
		...
		while ($p = $statement->fetchObject()){
			...
			$allViewableProfiles .= "<li class='groupList'><a href='index.php?page=wish-lists&amp;profile=$profile'>$Account->first_name $Account->last_name$thisIsTag</a></li>";
		}
		...
		while ($event = $statement->fetchObject()){
			...
			if ($host_id == $user_id){
				...
				$output .= "<form method='post' action='index.php?page=events'>
							<p><a href='$href'>$eventName</a> hosted by $host_name</p>
